...
> Nuage : 7 microservices Spring Boot + 1 service AI Python (FastAPI), streaming Kafka, frontend SPA.
...
flowchart LR
    ...
    subgraph Backend
        ...
        MAINT[maintenance-service :8086]
        AI[ai-service :8090]
    end
...
sequenceDiagram
    ...
    K-->>MAINT: consume telemetry.alerts
    MAINT->>MAINT: ouvrir OT & downtime
    ...
    UI->>AL: GET /api/v1/alerts/active
    AL-->>UI: alertes actives
    UI->>MAINT: GET /api/v1/maintenance/orders
    MAINT-->>UI: ordres de travail CMMS
```
